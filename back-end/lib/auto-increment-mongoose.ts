...
let counterSchema: Schema | null = null;

let IdentityCounter: Model<any> | null = null;

// Initialize plugin by creating counter collection in database.
export const initialize = (connection: typeof mongoose) => {
  try {
    IdentityCounter = connection.model("IdentityCounter");
    counterSchema = IdentityCounter.schema;
  } catch (ex) {
    if (ex instanceof MongooseError && ex.name === "MissingSchemaError") {
      ...
      counterSchema.index(
        ...
        {
          unique: true,
          //  required: true, // to check later
          // index: -1 // to check later
        }
      );
...
export const plugin = (schema: Schema, options: any) => {
  ...
  const fields: { [key: string]: { type: any; require: boolean; unique?: boolean } } = {}; // A hash of fields to add properties to in Mongoose.
  ...
  const nextCount = (callback: (err: any | null, count?: number) => void) => {
    IdentityCounter?.findOne({
      model: settings.model,
      field: settings.field,
    })
  ...
  const resetCount = (callback: (err: any | null, count?: number) => void) => {
